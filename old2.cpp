...
void moveFocus(int ch, size_t focusInitial){
  ...
  if(panelTree[parent].type){ // hsplit
    ...
    else if(ch == KEY_DOWN && first){
      found = 1;
    }    
  }
  else{
    if(ch == KEY_LEFT && !first){
      found = 1;
    }
    else if(ch == KEY_RIGHT && first){
      found = 1;
    }
  }
  if(found){
...
void fileDraw(FileView& fv, uint32_t x, uint32_t y, uint32_t w, uint32_t h){
  ...
  for(size_t line = 0; line < h; line++){
    ...
    size_t remainder = std::min(file.data.size()-ptr-1, (size_t)fv.columns);
...
size_t panelTreeDraw(size_t i, uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint32_t border = 0, bool drawSplit = false){
  ...
  if(w > COLS || h > LINES || x > COLS || y > LINES){
    printf("%d, %d\n", x, y);
    printf("%d, %d\n", w, h);
    return 1;
  }
  ...
  if(pt.isSplit){
    ...
    FileView& fv = pt.file;
    while(cursor/fv.columns > fv.scroll + h - 2 - ctx.scrollPadding){
      fv.scroll ++;
    }
    while(cursor/fv.columns < fv.scroll + ctx.scrollPadding && fv.scroll != 0){
      fv.scroll --;
    }
...
int main(int argc, char** argv){
  ...
  while(running){
    ...
    panelTreeDraw(0, 0, 0, COLS, LINES);
    move(LINES/2, 0);
    ...
    switch(ch){
      ...
      case 'w': {
        ...
        while(running){
          ...
          panelTreeDraw(0, 0, 0, COLS, LINES, 1, 1);
          ...
          switch(ch){
            ...
            case KEY_DOWN:
            case KEY_UP: {
              size_t p1 = findParent(ctx.focus);
              bool f1 = ctx.focus == p1+1;
              moveFocus(ch, ctx.focus);
              if(panelTree[ctx.focus].isSplit && panelTree[ctx.focus].type == panelTree[p1].type){
                ctx.focus ++;
                if(!f1) ctx.focus = findSibling(ctx.focus);
              }
              // else{
                fixFocus();
              // }
